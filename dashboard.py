...
# CSV линк дээр санамсаргүй тоо нэмж кэшээс зайлсхийх
import time
url = f"https://docs.google.com/spreadsheets/d/{sheet_id}/gviz/tq?tqx=out:csv&cachebust={int(time.time())}"

def load_data():
    # Streamlit-ийн кэшийг ашиглахгүйгээр шууд унших
    df = pd.read_csv(url)
...
try:
    ...
    # Даралт ба Пульс
    st.write("### 💓 Цусны даралт ба Пульс")
    ...
    # Хамгийн сүүлчийн датаны огноог харуулах
    st.success(f"Сүүлчийн өгөгдөл шинэчлэгдсэн: {latest['LogDate'].strftime('%Y-%m-%d')}")
